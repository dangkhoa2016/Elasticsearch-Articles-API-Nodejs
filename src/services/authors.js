const debug = require('debug')('elasticsearch-articles-api-nodejs:Services->authors');
const { models: { Author, Authorship } } = require('../models');
...
class AuthorService {
  ...
  async getAll(offset = 0, limit = 10, filter = {}, replacements = [], showTotal = false) {
    ...
    try {
      const query = {
        where: filter, replacements
      };
      const authors = await Author.findAll({ ...query, offset, limit, order: [['first_name', 'asc']] });
      if (stringToBoolean(showTotal))
        return { data: authors, total: await Author.count(query) };
      else
        return authors;
    } catch (err) {
  ...
  async getByField(fieldName, fieldValue) {
    ...
    try {
      return await Author.findOne({ where });
    } catch (err) {
  ...
  async delete(id) {
    if (!id) {
      debug('Author id is empty.');
      return;
    }

    var record = await Author.findByPk(id);
    if (!record) {
      debug(`Author with id: [${id}] does not exists.`);
      return;
    }

    try {
      var articles = await record.getArticles();
      await Authorship.destroy({ where: { author_id: id } });
